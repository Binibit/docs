...
An object with a top-level `market_pairs` array. Each element describes one pair.

<ResponseField name="market_pairs" type="array" required>
...
const { market_pairs } = await res.json();
```
