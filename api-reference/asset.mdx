...
Returns metadata for every asset listed on Binibit.

<Info>
  This endpoint is **not part** of the CoinGecko Integration API Standards v8 for spot exchanges. It is provided for convenience and parity with CoinMarketCap's `/assets` endpoint.

  CoinGecko aggregator integration relies only on `/tickers`, `/orderbook`, and `/historical_trades`.
</Info>
